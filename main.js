    ...
    sections: [
      {
        heading: '動機',
        body: `
        印刷電路板（PCB）是各式電子產品中最關鍵的基板，只要板上元件裝配錯誤或焊接缺陷，就可能導致整機異常，因此生產過程中的缺陷檢測是品質管控的重要環節。 傳統 AOI 結合人工覆判的檢測流程，不僅設備成本高、人工訓練時間長，且容易因光影變化與長時間作業導致誤判與過檢。本專題希望透過深度學習建置 PCB 影像分類模型，先自動判斷元件類型，作為後續瑕疵分類與 AI 覆判系統的基礎。
        `
